 ...
 */
@Slf4j
public class ChatClientWebSocketHandler extends ChannelInboundHandlerAdapter {

    public ChatClientWebSocketHandler(WebSocketClientHandshaker clientHandshaker) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        System.out.println("channelRead0  " + this.clientHandshaker.isHandshakeComplete());
        Channel ch = ctx.channel();
        FullHttpResponse response;
        if (!this.clientHandshaker.isHandshakeComplete()) {
            try {
                response = (FullHttpResponse) msg;
                //握手协议返回，设置结束握手
                this.clientHandshaker.finishHandshake(ch, response);
                //设置成功
