...
public class RabbitConsumer1 {
    ...
    public static void main(String[] args) {
        RabbitConsumer1 consumer = new RabbitConsumer1();
//        consumer.directConsumer();
//        consumer.fanoutConsumer_0();
        consumer.topicConsumer_0();
    }
...
//            channel.basicConsume(RabbitDetailsEnum.DIRECT_LOG.getQueue(), true, consumer);
    ...
    public void fanoutConsumer_0() {
        RabbitMqConfiguration configuration = new RabbitMqConfiguration();
        Channel channel = configuration.channel;
        try {
            String exchange = RabbitDetailsEnum.FANOUT_API_0.getExchange();
            String exchangeType = RabbitDetailsEnum.FANOUT_API_0.getExchangeType();
            String queue = RabbitDetailsEnum.FANOUT_API_0.getQueue();
...
//            channel.basicConsume(queue, true, consumer);
    ...
    /**
     * Topic Exchange Topic类型的交换器在匹配规则上进行了扩展，它有一定的约束条件：
     * 1、RoutingKey 为一个点号" . “分隔的字符串(被点号”.“分隔开的每段独立的字符串称为一个单词) 如：com.toher.user 和 org.toher.product
     * 2、BindingKey和RoutingKey同样也是点号”."分隔的字符串;
     * 3、BindingKey 中可以存在两种特殊字符串 * 和 #，用于做模糊匹配，其中 * 代表一个单词，# 可以用于匹配多规格单词(可以是零个)
     */
    public void topicConsumer_0() {
        RabbitMqConfiguration configuration = new RabbitMqConfiguration();
        Channel channel = configuration.channel;
        try {
            String exchange = RabbitDetailsEnum.TOPIC_LOG_0.getExchange();
            String exchangeType = RabbitDetailsEnum.TOPIC_LOG_0.getExchangeType();
            String queue = RabbitDetailsEnum.TOPIC_LOG_0.getQueue();
            String routingKey = "fx_#";
            channel.exchangeDeclare(exchange, exchangeType, true, false, false, null);
            channel.queueDeclare(queue, true, false, false, null);
            //            绑定交换机
            channel.queueBind(queue, exchange, routingKey);
